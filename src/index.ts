...
(async () => {
  ...
  for (const link of cleanedLinks) {
    ...
    try {
      ...
      reviews = reviews?.replace(' mil avaliações', '')
      reviews = parseInt(reviews!) * 1000
    ...
    try {
      ...
      if (installs?.indexOf('star') === -1) {
        installs = await page.$eval('#yDmH0d > c-wiz.SSPGKf.Czez9d > div > div > div.tU8Y5c > div.P9KVBf > div > div > c-wiz > div.hnnXjf.XcNflb.J1Igtd > div.JU1wdd > div > div > div:nth-child(2) > div.ClM7O', (item) => { return item.textContent?.replace('+', '') })
      }
      installs = installs!.replace(' mil', '')
      installs = installs!.replace('mil', '')
      installs = parseInt(installs) * 1000
